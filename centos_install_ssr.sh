...
BASE=`pwd`#!/bin/bash
# shadowsocksR/SSR CentOS 7/8一键安装教程
# Author: hijk<https://www.github.com>
...
if [[ "$?" != "0" ]]; then
    IP=`curl -sL -6 ip.sb`
    V6_PROXY="https://www.github.com/"
fi

FILENAME="ShadowsocksR-v3.2.2"
URL="${V6_PROXY}http://www.6jhw.com/dns/shadowsocksr-3.2.2.tar.gz"
BASE=`pwd`
...
slogon() {
    ...
    echo -e "# ${GREEN}作者${PLAIN}: 网络跳动(Erick)                                      #"
    echo "#############################################################"
    echo ""
}
...
installSSR() {
    if [ ! -d /usr/local/shadowsocks ]; then
        ...
        if [ ! -f /usr/local/shadowsocks/server.py ]; then
            colorEcho $RED " $OS 安装失败，请到 https://www.github.com 网站反馈"
